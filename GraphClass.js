export default class GraphClass {
    ...
    */
        let s = JSON.stringify(this.graph.nodes.map(n => n.id)) + JSON.stringify(this.graph.edges.map(e => e.source.id + e.target.id));
    ...
    */
        ...
        let subEdges = this.graph.edges.filter(e => component.has(indexMap.get(e.source)) && component.has(indexMap.get(e.target)));
    ...
    */
    ...
    // Problem HW4 Compute average shortest path length
    computeAPL() {
        let A = this.computeAPSP();
        let flattenedA = A.flat();
        // Filter out all elements that are not equal to -1
        let filteredA = flattenedA.filter(x => x != -1);
        let n = filteredA.length - A.length;
        // Flatten the 2D array 'A' into a 1D array
        filteredA = flattenedA.filter(x => x != -1);
        let new_sum = filteredA.reduce((accumulator, currentValue) => {
